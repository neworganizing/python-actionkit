# copied from python-actionkit which doesn't support this version of Django yet
...
class AKMailerAPI(ActionKitAPI):
    ...
    def create_mailing(self, mailing_dict):
        ...
        res = self.client.post(
            '%s/rest/v1/mailer/' % self.base_url,
            json=mailing_dict)
        rv = {'res': res}
    ...
    def get_mailings(self, **kwargs):
        if getattr(self.settings, 'AK_TEST', False):
            return TEST_DATA.get(1234)
        params = []
        for key, value in kwargs.items():
            params.append('='.join([key,value]))
        if params:    
            res = self.client.get(
                '{}/rest/v1/mailer/?{}'.format(self.base_url, '&'.join(params)))
        else:
            res = self.client.get(
                '{}/rest/v1/mailer/'.format(self.base_url))
        return {'res': res,
